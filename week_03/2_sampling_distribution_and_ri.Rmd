...
Suppose that we are working through the example of a randomized
...
to treatment, we multiply the potential outcome to treatment times the
...
outcome to control times the quantity `(1 - treatment)` which will be one
...
# Conducting an experiment by sampling (once) the treatment vector
