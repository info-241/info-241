...
This is a model that has *only* an intercept, which is indicated in the `1` in the formula call. Note also that just as we are able to perform relatively simple functions on the column space using `data.table` (e.g. `d[ , mean(x)]`) we can also perform relatively more complex functions -- like a `t.test` or a `lm`. Neato. 
...
Here, we have no evidence to suggest that the additional model terms increased the accuracy of the model's predictions. Indeed the p-value is nearly 0.5, scant evidence in support of the alternative hypothesis. 
...
probs <- list(c(0.5, 0.5),              # create a list of treatment assignment 
              c(0.1, 0.9),              # probabilities. we're going to step 
...
for(b in 1:3) {                         # start a loop from 1:3
    d[block == blocks[b],               # for each iteration, match against
      ':='(treat = sample(c(0,1), .N,   # the objects 'blocks' in position i
               ...
               prob = probs[[b]]),      # drawn from the probs list at index i
           x1 = rnorm(.N, mean = b),
           x2 = rbinom(.N, 2*b, .25),
...
How might you similarly conduct your `anova` test within blocks? 
