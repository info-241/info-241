...
Suppose that we have 1,000 individuals that we can observe in our study. Furthermore, suppose that for each of these people, there is some *latent*, or unmeasured, response to treatment that they *would* have to treatment if we assigned them to take the treatment. 
...
Make two new columns, using the `:=` assignment operator in the data.table. We'll make the first column for you. You produce the second column. 
...
And so, we can represent this **science fiction table** -- the set of all potentially realizable outcomes, and also measurable covariates. 
...
The first of these is easy: For every subject, no matter whether they are in treatment or control, we measure the trait that we care about. But, the second two require considerably more care, and are the focus of the class. 

In this experiment, the way that we are intervening in people's lives is to either give them *coffee* or *decaf coffee*. Then, and only then, we're also able to make some of their potential outcomes measurable. 
...
And, so, while the *science fiction table* might contain information about each persons potential outcomes to treatment and control, the table of data that we're ever going to be able to generate has a more restricted set. Call this observable data set `obs`, and it is a subset of all the data that might be out there.
...
2. Among people who are in treatment, what is the mean potential outcome to control (`y_0`)? Is this any different than the people who are in control? 
...
But, though random assignment, the values for the potential outcomes to control that we see *should* be an unbiased estimate of the whole populations potential outcomes to control. 
...
The mean of `y0` is `r round(mean_y0, 3)`, and the mean of control Y is `r round(mean_control_Y, 3)`. 
...
Because the *observable* set of realized potential outcomes are unbiased estimates of the *unobservable* potential outcomes, we're able to generate an unbiased estimate of the causal effect, using only observable data! 

- Calculate the mean of the observable potential outcomes to treatment. 
- Calculate the mean of the observable potential outcomes to control. 
...
Let that sink in a bit. Most of your job as a data scientist *not* in producing some results; most of your job as a data scientist is in producing results that are the *right* results, and that requires thought. 