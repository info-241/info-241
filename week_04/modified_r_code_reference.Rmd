...
If we call this function once, the function returns back the estimated average treatment effect in the simulated study and the number of women that were assigned to treatment (out of the possible 20). 
...
We set the study up so that we *know* there is no treatment effect. And, when we have 10 women who we randomize into control we, indeed, recover this absence of an effect. Our estimates for the ATE seem pretty well centered around zero. 
...
Sometime there are design, practical, or ethical constrains that mean that although we can observe the outcome of treatment for each individual, we can't actually *individually* assign each student to a treatment/control level. 
...
1. **School day length**: We can observe the schooling performance at the individual level, but we can't randomly assign Timmy to have a long day and Tommy to have a short day. 
...
If you wanted to see the classroom ids next to these assignment vector, you could just hack the two `classroom_ids` and `randomize_clustered()` together using `table`. 

```{r}
table(randomize_clustered(), classroom_ids)
```

Now that we're sure it "works", let's actually make the treatment assignment vector, and the potential outcomes to treatment and control. 
...
And we can pull out the p-value under the randomization inference framework by examining the percent of the values that occur at a place that is smaller/larger than the p-value we observe using `mean(ate < distribution_under_sharp_null)` which evaluates to `r mean(ate < distribution_under_sharp_null)`. 

## What if we ignore clustering?

At this point -- after doing the reading and participating in the async material -- you might be thinking, "But guys, having to estimate the weighted ATE for every cluster, and then aggregate it back into an estimate for the whole population is hard. And that SE formula with groups? C'mon..."

And you're right, it is more work when we have grouping in our data that diverge from the case of simple random assignment across a homogeneous population. However, if you ignore the clustering that exists in your data, you may (read: "are likely to") badly misstate the treatment effect because your estimator ignoring the clustering is not guaranteed to be an unbiased estimator. 
...
Under the *wrong* setup, we have an inappropriately tight distribution surrounding the ATE under the sharp null. This will lead to us rejecting the null hypothesis that there is no effect in cases where we should *not* actually reject that null hypothesis. This is Type-1 error. 
...
Why does this happen? There are a number of reasons you might be inappropriately *too precise*. 
...
2. You're ignoring classroom level noise that might be leading to different outcomes. If there are different propensities for some classrooms to do better, or have higher potential outcomes, then because you've got a small number of `k`, you're going to be wrong! 
