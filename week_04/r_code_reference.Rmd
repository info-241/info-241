...
m
``` 
And so we we see that there is a `r m` probability of observing an ATE of this size, given the repeated randomization regime, under the sharp null hypothesis. 
...
``` 
That line is kind of warbling around as a result of statistical noise, but more or less, we have a horizontal line -- or at the very least, a line that would not be very informative if we had to place a bet about where on the range the p-value is most likely to lie. 
...
1. The actual treatment effect $(\tau_{i})$ is an effect that is a parameter of the *real* world, something that is unobserved
...
Second: let's work with the other lever that we actually *can* pull, manipulating the effect size. Even if we can't move $\tau$, we can enroll 1x, 5x, or 50x the number of people in our study. 
