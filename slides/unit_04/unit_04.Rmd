...
## Take care
- Take care to notice how this statement of the sharp null hypothesis is different from the guarantees of apples-to-apples comparisions that are achieved through randomization
...
cluster_ri <- function(x_data, clustered) { 
  ...
  if(clustered == TRUE) { 
    ...
    d[ , .(group_mean = mean(Y)), 
       keyby = .(ri_classroom_assignment)][ , diff(group_mean)]    
  } else if(clustered == FALSE) {
    d <- x_data
    
    d[ , .(group_mean = mean(Y)),
       keyby = .(sample(classroom_assignment))][ , diff(group_mean)]
  }
}
```
...
- To account for this uncertainty, when conducting randomization inference, *match the experiment's assignment process*
  - If experimental assignment happened at random, randomization inference needs to as well
  - If experimental assignment happened with clusters, randomization inference needs to as well;
  - If experimental assignment happened with blocks, randomization inference needs to as well.
