...
In the world that we've occupied to this point, whether we *do* or *do not* condition on a variable, an experiment that we've executed that has successfully randomized the treatment should produce a reliable causal estimate. That is, in this setup, **if we estimate a model, it should provide us with an unbiased estimate of $\tau$, or 2.**. 
...
And, finally, create the observed outcomes. 
```{r} 
d[D == 0, Y := y0 + bc]
d[D == 1, Y := y0 + bc + tau]
```
...
In fact, we can estimate a reliable causal effect for *either* `D` on `Y`, *or* `D` on `bc`, but not the two together. 


First, the causal effect of `D` on `Y` in \autoref{goodModel}. 
```{r, results='asis'}
m1 <- lm(Y ~ D, data = d)
...
Now we're left with an under-estimate of the causal effect of `D` on `Y`, and we've got some estimate of the effect of `bc` on `Y`. But, when we built the data, there wasn't such an effect! In fact, what is in here is the relationship between `y0` and `Y`, but through a mangled causal pipeline. 
...
The Dean, in her wisdom, agrees to let current students and 241 faculty run an experiment where they randomly admit people to the program. Hurrah! We can evaluate this! And we can make a causal claim. 

Suppose we have the following as a setup: 

- Some students live in Bay Area. Because wages are stupid here (so are mortgages), these students are paid *on average* $25,000 more than people who do not live in the Bay Area. 
...
- There are some students who are *really* motivated to get that job at Pinterest. And, they think that actually have a shot at it once they're in MIDS! So, of the folks who are admitted, suppose that 25% of them move to the Bay Area. None of the people who are not admitted to MIDS move to the Bay Area. 
...
## And for the people who already lived in SF, they get the treatment effect 
